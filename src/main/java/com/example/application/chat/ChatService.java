...
public class ChatService {
  ...
  public ChatService(
      ChannelRepository channelRepository, MessageRepository messageRepository, Clock clock) {
  ...
  private void generateTestData() {
    ...
    for (String channelName : chatChannels) {
      ...
      log.info(
          "Created channel: {} (http://localhost:8080/channel/{})", channel.name(), channel.id());
    }
  }
  ...
  public List<Message> messageHistory(
      String channelId, int fetchMax, @Nullable String lastSeenMessageId) {
  ...
  public void postMessage(String channelId, String message) throws InvalidChannelException {
    ...
    final String author = SecurityContextHolder.getContext().getAuthentication().getName();
